...
def calculate_average_value(rolling_avrg_value: int, df: list):
    """
    values sütunundaki değerleri birer,ikişer,üçer.... şekilde toplayıp ortlamasını alıp yeni sütununda gerekli
    konumunda yazıyor.

    :param rolling_avrg_value: int value of rolling average value
    ...
    for i in range(len(df)):
        ...
        if i <= rolling_avrg_value:
            b.append(None)
        else:
            sub_df = df["values"][i - 1 - rolling_avrg_value: i + 1]
            sum_df = sum(sub_df)
            len_df = len(sub_df)
            res = sum_df / len_df
            b.append(res)

    df[f"avg_{rolling_avrg_value + 2}"] = b
